...
const SiteMarker = ({ place , i}) => {
  ...
  return (
    <Marker position={[place.location.latitude, place.location.longitude]} icon={Icon}>
      <Popup>
        <div className="siteMarker-popup">
          <h2>{place.name}</h2>
          <p>{truncateAfterWord(place.description,83)}</p>
          <Button type="primary" block onClick={()=>setVisibleModalDetail(true)}>
            Más Info
          </Button>
        </div>
        <ModalSiteDetail visibleModalDetail={visibleModalDetail} setVisibleModalDetail={setVisibleModalDetail} place={place}/>
      </Popup>
    </Marker>
  );
};
